...
def model_collapsed(eta, t):
    # eta can be a weird shape
    L_1, theta_0_1, G_1, phi_1, L_2, theta_0_2, G_2, phi_2 = eta
    ...
    model_1 = L_1 * np.sin(theta_0_1 * np.cos(np.sqrt((G_1 * phi_1) / L_1)*t))
    model_2 = L_2 * np.sin(theta_0_2 * np.cos(np.sqrt((G_2 * phi_2) / L_2)*t))
...
offset_position = model(eta_o, time)
...
pos = start + np.array([[2e-1,1e-1,1e0,1e0],[2e-1,1e-1,1e0,1e0]]).reshape((4,2)) * np.random.randn(10, 4, 2)
'''
